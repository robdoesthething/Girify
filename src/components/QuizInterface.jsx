...
const QuizInterface = ({
  ...
  const getPrefix = name => {
    ...
    const match = name.match(
      /^(Carrer|Avinguda|Plaça|Passeig|Passatge|Ronda|Via|Camí)(\s+d(e|els|es)?)?\s+/i
    );
  ...
  return (
    <div className="w-full h-full flex flex-col pointer-events-auto overflow-hidden">
      ...
      >
        ...
        {/* Hints Section */}
        <div
          className={`p-2 md:p-4 border-t ${theme === 'dark' ? 'border-slate-700 bg-slate-800' : 'border-slate-100 bg-slate-50'}`}
        >
