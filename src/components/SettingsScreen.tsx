...
const SettingsScreen: React.FC<SettingsScreenProps> = ({
  ...
  setAutoAdvance: setAutoAdvanceProp,
  username,
}) => {
  ...
  // Inline notification support check (replaces deleted useNotifications hook)
  const isIOS =
    /iPad|iPhone|iPod/.test(navigator.userAgent) ||
    (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
  const isSupported = 'Notification' in window && !isIOS;
  const requestPermission = () => Notification.requestPermission().then(r => r === 'granted');

  // autoAdvance: use prop if provided, otherwise manage locally
  const [localAutoAdvance, setLocalAutoAdvance] = useState(false);
