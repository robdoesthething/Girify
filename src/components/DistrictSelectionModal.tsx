...
function DistrictSelectionModal({ username, onComplete, onDismiss }: DistrictSelectionModalProps) {
  ...
  return (
    <Modal
      ...
      contentClassName={`p-8 rounded-3xl shadow-2xl border ${themeClasses(theme, 'bg-slate-900 border-slate-700 text-white', 'bg-white border-slate-200 text-slate-900')}`}
    >
      {onDismiss && (
        <button
          onClick={onDismiss}
