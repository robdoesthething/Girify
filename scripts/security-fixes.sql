...
WITH CHECK (
  user_id IN (SELECT username FROM users WHERE supabase_uid = auth.uid()::text)
);
...
FOR INSERT TO authenticated
WITH CHECK (
  username IN (SELECT username FROM users WHERE supabase_uid = auth.uid()::text)
);
...
BEGIN
  ...
  IF auth.uid() IS NOT NULL THEN
    IF NOT EXISTS (
      SELECT 1 FROM users WHERE username = p_username AND supabase_uid = auth.uid()::text
    ) THEN
