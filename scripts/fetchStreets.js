...
const query = `
    ...
    (
      way["highway"~"^(primary|trunk|secondary|tertiary|residential)$"]["name"](area.searchArea);
    );
    out geom;
`;
...
async function fetchStreets() {
    ...
    try {
        ...
        data.elements.forEach(element => {
            ...
            // Normalize coordinate to [lat, lon]
            const geometry = element.geometry.map(p => [p.lat, p.lon]);
        ...
        for (let t = 1; t <= 4; t++) {
            ...
            finalStreets.push(...tierStreets.slice(0, 50));
        }
